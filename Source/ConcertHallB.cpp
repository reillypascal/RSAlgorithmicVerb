...
void LargeConcertHallB::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
    {
        ...
        for (int channel = 0; channel < (buffer.getNumChannels() <= 2 ? buffer.getNumChannels() : 2); ++channel)
        {
            ...
            // IN FROM BUFFER
            inputBandwidth.pushSample(channel,
                                      (buffer.getSample(channel, sample) * 0.812) + (channelInput.at(channel) * 0.188));
        ...
        // start output L
        channelOutput.at(0) =
            (loopDelayL3.getSampleAtDelay(0, 276) * 0.938) - (loopDelayL3.getSampleAtDelay(0, 312) * 0.438);
        // output R
        ...
        feedforwardInner = reverbData[sample] * -0.25 * parameters.diffusion;
        // innermost
        allpassOutputInnermost = allpassL4Innermost.popSample(
            0, allpassL4InnermostSize + (lfoOutput.normalOutput * 150.0f * parameters.modDepth)); // modulate
        // one-pole lowpass - rename delays later
        allpassChorusL.pushSample(0, allpassOutputInnermost);
        allpassOutputInnermost *= 0.781;
        //        allpassOutputInnermost += allpassChorusL.popSample(0, scale(lfoOutput.normalOutput,
        //        -1.0f, 1.0f, 1.0f, 12.0f * parameters.modDepth)) * 0.219; // modulate here
        ...
        // output R
        channelOutput.at(1) +=
            (loopDelayL4.getSampleAtDelay(0, 36) * 0.469) + (loopDelayL4.getSampleAtDelay(0, 1572) * 0.125);
        ...
        channelOutput.at(1) +=
            (loopDelayR3.getSampleAtDelay(0, 24) * 0.938) - (loopDelayR3.getSampleAtDelay(0, 192) * 0.438);
        ...
        feedforwardInner = reverbData[sample] * -0.25 * parameters.diffusion;
        // innermost
        allpassOutputInnermost = allpassR4Innermost.popSample(
            0, allpassR4InnermostSize + (lfoOutput.quadPhaseOutput_pos * 150.0f * parameters.modDepth)); // modulate
        // one-pole lowpass - rename delays later
        allpassChorusR.pushSample(0, allpassOutputInnermost);
        allpassOutputInnermost *= 0.781;
        //        allpassOutputInnermost += allpassChorusL.popSample(0, scale(lfoOutput.quadPhaseOutput_pos,
        //        -1.0f, 1.0f, 1.0f, 12.0f * parameters.modDepth)) * 0.219; // modulate here
        allpassOutputInnermost += allpassChorusL.popSample(0) * 0.219;
        // finish innermost
        feedbackInnermost = allpassOutputInnermost * 0.25 * parameters.diffusion;
        ...
        for (int destChannel = 0; destChannel < buffer.getNumChannels(); ++destChannel)
        {
            if (destChannel < 2)
            {
                buffer.setSample(
                    destChannel, sample,
                    channelOutput.at(destChannel) *
                        3.0); // scale here, since output scalars are numerous and I don't want to mess with the ratio
            }
        }
    }
}
...
ReverbProcessorParameters& LargeConcertHallB::getParameters()
{
    return parameters;
}
...
//			channelInput.at(channel) = inputBandwidth.popSample(channel);
//			// IN FROM BUFFER
//			inputBandwidth.pushSample(channel, (buffer.getSample(channel, sample) * 0.812) + (channelInput.at(channel) *
//0.188)); 			inputZ.pushSample(channel, channelInput.at(channel)); 			channelInput.at(channel) = inputZ.popSample(channel) *
//0.5;
//
//			// feedback filtering, mix of feedback/input
...
//		reverbData[sample] = loopDelayL3.popSample(0);
//		// start output L
//		channelOutput.at(0) = (loopDelayL3.getSampleAtDelay(0, 276) * 0.938) - (loopDelayL3.getSampleAtDelay(0, 312) *
//0.438);
//		// output R
//		channelOutput.at(1) += loopDelayL3.getSampleAtDelay(0, 40) * 0.438;
//
...
//		allpassChorusL.pushSample(0, allpassOutputInnermost);
//		allpassOutputInnermost *= 0.781;
//		allpassOutputInnermost += allpassChorusL.popSample(0, scale(lfoOutput.normalOutput, -1, 1, 1, 12)) * 0.219; //
//modulate here
//		// finish innermost
...
//		reverbData[sample] = loopDelayL4.popSample(0);
//		// output R
//		channelOutput.at(1) += (loopDelayL4.getSampleAtDelay(0, 36) * 0.469) + (loopDelayL4.getSampleAtDelay(0, 1572) *
//0.125);
//
//		// feedback *TO* R channel
//		channelFeedback.at(1) = reverbData[sample];
...
//		// output L
//		channelOutput.at(0) += loopDelayR3.getSampleAtDelay(0, 468) * 0.438;
//		channelOutput.at(1) += (loopDelayR3.getSampleAtDelay(0, 24) * 0.938) - (loopDelayR3.getSampleAtDelay(0, 192) *
//0.438);
//
//		// allpass 4
//		// begin outer
...
//		allpassChorusR.pushSample(0, allpassOutputInnermost);
//		allpassOutputInnermost *= 0.781;
//		allpassOutputInnermost += allpassChorusL.popSample(0, scale(lfoOutput.quadPhaseOutput_pos, -1, 1, 1, 12)) *
//0.219; // modulate here
//		// finish innermost
//		feedbackInnermost = allpassOutputInnermost * 0.25 * mDiffusion;
//		reverbData[sample] += feedbackInnermost;
