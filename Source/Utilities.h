...
struct ReverbProcessorParameters
{
    ...
    {
        if (params.damping == damping && params.decayTime == decayTime && params.diffusion == diffusion &&
            params.modDepth == modDepth && params.modRate == modRate && params.roomSize == roomSize)
            return true;
