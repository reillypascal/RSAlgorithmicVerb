...
RSAlgorithmicVerbAudioProcessor::RSAlgorithmicVerbAudioProcessor()
    ...
    : AudioProcessor(BusesProperties()
      ...
      parameters(
          *this, nullptr, juce::Identifier("RSAlgorithmicVerb"),
          {std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"roomSize", 1}, "Room Size", 0.0f, 1.0f, 0.5f),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"preDelay", 1}, "Pre-Delay",
                                                       juce::NormalisableRange<float>(0.0f, 150.0f, 1.0f), 0.0f),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"feedback", 1}, "Feedback", 0.0f, 0.99f,
                                                       0.35f),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"damping", 1}, "Damping", 0.0f, 1.0f, 0.5f),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"diffusion", 1}, "Diffusion", 0.0f, 0.99f,
                                                       0.67f),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"earlySize", 1}, "Early Size", 0.0f, 1.0f,
                                                       0.5f),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"earlyDecay", 1}, "Early Decay", 0.0f, 0.99f,
                                                       0.35f),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"modRate", 1}, "Mod Rate", 0.0f, 5.0f, 2.25f),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"modDepth", 1}, "Mod Depth", 0.0f, 1.0f, 0.5f),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"highCut", 1}, "High Cut",
                                                       juce::NormalisableRange<float>(200, 20000, 5), 20000),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"lowCut", 1}, "Low Cut",
                                                       juce::NormalisableRange<float>(20, 1000, 5), 20),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"earlyLateMix", 1}, "Early Reflections Mix",
                                                       0.0f, 1.0f, 0.0f),
           std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"dryWetMix", 1}, "Dry/Wet Mix", 0.0f, 1.0f,
                                                       0.35f),
           std::make_unique<juce::AudioParameterChoice>(
               juce::ParameterID{"reverbType", 1}, "Reverb Type",
               juce::StringArray{"DattorroPlate", "DattorroHall", "SmallRoom", "MediumRoom", "LargeRoom", "Freeverb",
                                 "Anderson8xFDN", "circulant8xFDN", "Hadamard8xFDN", "Householder8xFDN",
                                 "Constellation", "EventHorizon"},
               0)})
{
}
...
void RSAlgorithmicVerbAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    *lowCutFilter.state = *juce::dsp::IIR::Coefficients<float>::makeHighPass(
        getSampleRate(), parameters.getRawParameterValue("lowCut")->load());
    *highCutFilter.state = *juce::dsp::IIR::Coefficients<float>::makeLowPass(
        getSampleRate(), parameters.getRawParameterValue("highCut")->load());
    ...
    earlyReflections.setMonoFlag(
        earlyMonoFlagsPerProcessor[static_cast<juce::AudioParameterChoice*>(parameters.getParameter("reverbType"))
                                       ->getIndex()]);
    ...
    if (reverbProcessor != nullptr)
    {
        ...
        reverbParameters.damping =
            scale(parameters.getRawParameterValue("damping")->load() * -1.0f + 1.0f, 0.0f, 1.0f, 200.0f, 20000.0f);
