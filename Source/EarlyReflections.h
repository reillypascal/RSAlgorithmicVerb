...
class EarlyReflections : public ReverbProcessorBase
{
  ...
  private:
    ...
    // early reflections mono/stereo - prevents comb filtering on reverbs that mix input to mono; PluginProcessor sets
    // mono/stereo by processor
    bool monoFlag = false;
};
...
//			channel0Output += earlyReflectionsDelayLine.getSampleAtDelay(0, 6319 * mSize) * mInitialLevel * pow(mDecay,
//2);
//
...
//			channel1Output += earlyReflectionsDelayLine.getSampleAtDelay(0, 9462 * mSize) * mInitialLevel * pow(mDecay,
//2);
//
